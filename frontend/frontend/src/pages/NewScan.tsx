...
import { ScanFace, RotateCcw, AlertTriangle, Loader2, ShieldCheck } from "lucide-react";
    ...
    <div className="space-y-6">
      ...
      {started && (
        <div className="space-y-6">
          ...
          {!state.error && state.match?.status === "original_content" && (
            <div className="flex items-start gap-2 rounded-lg border border-accent-blue/30 bg-accent-blue/5 px-4 py-3 text-sm text-slate-200">
              <ShieldCheck size={16} className="mt-0.5 shrink-0 text-accent-blue" />
              <div>
                {state.match.note ||
                  "No similar public image was found. Your uploaded content was fingerprinted, registered on-chain, and verified as original."}
              </div>
            </div>
          )}
